...
void AtomVecDemsi::grow(int n)
{
  ...
  ridgingIceThickness = memory->grow(atom->ridgingIceThickness,nmax,"atom:ridgingIceThickness");
  ridgingIceThicknessWeight = memory->grow(atom->ridgingIceThicknessWeight,nmax,"atom:ridgingIceThicknessWeight");
  netToGrossClosingRatio = memory->grow(atom->netToGrossClosingRatio,nmax,"atom:netToGrossClosingRatio");
  changeEffectiveElementArea = memory->grow(atom->changeEffectiveElementArea,nmax,"atom:changeEffectiveElementArea");
...
void AtomVecDemsi::grow_reset()
{
  ...
  ridgingIceThickness = atom->ridgingIceThickness;
  ridgingIceThicknessWeight = atom->ridgingIceThicknessWeight;
  netToGrossClosingRatio = atom->netToGrossClosingRatio;
  changeEffectiveElementArea = atom->changeEffectiveElementArea;
...
void AtomVecDemsi::copy(int i, int j, int delflag)
{
  ...
  ridgingIceThickness[j] = ridgingIceThickness[i];
  ridgingIceThicknessWeight[j] = ridgingIceThicknessWeight[i];
  netToGrossClosingRatio[j] = netToGrossClosingRatio[i];
  changeEffectiveElementArea[j] = changeEffectiveElementArea[i];
                               ...
                               int pbc_flag, int *pbc)
{
  ...
  buf[m++] = ridgingIceThickness[j];
  buf[m++] = ridgingIceThicknessWeight[j];
  buf[m++] = netToGrossClosingRatio[j];
  buf[m++] = changeEffectiveElementArea[j];
  buf[m++] = ice_area[j];
  buf[m++] = coriolis[j];
  buf[m++] = ocean_vel[j][0];
  buf[m++] = ocean_vel[j][1];
...
ridgingIceThickness[i] = buf[m++];
ridgingIceThicknessWeight[i] = buf[m++];
netToGrossClosingRatio[i] = buf[m++];
changeEffectiveElementArea[i] = buf[m++];
ice_area[i] = buf[m++];
coriolis[i] = buf[m++];
ocean_vel[i][0] = buf[m++];
ocean_vel[i][1] = buf[m++];
...
int AtomVecDemsi::pack_exchange(int i, double *buf)
{
  ...
  buf[m++] = ridgingIceThickness[i];
  buf[m++] = ridgingIceThicknessWeight[i];
  buf[m++] = netToGrossClosingRatio[i];
  buf[m++] = changeEffectiveElementArea[i];
  buf[m++] = ice_area[i];
  buf[m++] = coriolis[i];
  buf[m++] = ocean_vel[i][0];
  buf[m++] = ocean_vel[i][1];
...
ridgingIceThickness[nlocal] = buf[m++];
ridgingIceThicknessWeight[nlocal] = buf[m++];
netToGrossClosingRatio[nlocal] = buf[m++];
changeEffectiveElementArea[nlocal] = buf[m++];
...
rmass[nlocal] = MY_PI*radius[nlocal]*radius[nlocal];
  ...
  ridgingIceThickness[nlocal] = 0.0;
  ridgingIceThicknessWeight[nlocal] = 0.0;
  netToGrossClosingRatio[nlocal] = 0.0;
  changeEffectiveElementArea[nlocal] = 0.0;
  ice_area[nlocal] = 0.0;
  coriolis[nlocal] = 0.0;
