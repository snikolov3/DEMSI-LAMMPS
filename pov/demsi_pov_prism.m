...
%          fprintf(fidW1, '}\n');
    ...
    for w1 = 1:length(triplets(:,1))
            ...
            fprintf(fidW1, '\nprism{ 0.00, -thickness1, 4 <%f, %f>, <%f, %f>, <%f, %f>, <%f, %f> texture{pigment{ color rgb< 1, 1, 1>*0.85 } finish {ambient 0.2 diffuse 0.6 phong .75 phong_size scaleP1}} rotate<-90,0,0>}\n', ...  %texture { White_Marble scale scaleP1 } }\n', ... 
                particle_info(particle_info(:,1) == triplets(w1,1), 3), particle_info(particle_info(:,1) == triplets(w1,1), 4), ...
                particle_info(particle_info(:,1) == triplets(w1,2), 3), particle_info(particle_info(:,1) == triplets(w1,2), 4), ...
                particle_info(particle_info(:,1) == triplets(w1,3), 3), particle_info(particle_info(:,1) == triplets(w1,3), 4), ...
                particle_info(particle_info(:,1) == triplets(w1,1), 3), particle_info(particle_info(:,1) == triplets(w1,1), 4));
